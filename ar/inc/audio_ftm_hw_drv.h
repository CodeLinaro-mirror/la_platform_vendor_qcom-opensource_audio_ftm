...
extern int g_curr_ext_lb_device;
...
#define N_FFT 512
extern unsigned int g_analyze_status;
extern unsigned int g_freq_tested;
extern unsigned int g_total_freq_to_test ;
extern unsigned int g_cap_freq;
...
typedef enum
{
  AUDIO_FTM_AFE_LOOPBACK = 1,
	AUDIO_FTM_ADIE_LOOPBACK,
	AUDIO_FTM_EXT_LOOPBACK
...
typedef struct _Aud_FTM_DevCtxt
{
   ...
   int32                      period_size;
	 ...
	 char*   gkv_rx;
	 char*   be_name_rx;
         unsigned int device_id_rx;
         unsigned int channels_rx;
	 char*   gkv_tx;
	 char*   be_name_tx;
         unsigned int device_id_tx;
         unsigned int channels_tx;
} Aud_FTM_DevCtxt_T;
...
int audio_ftm_hw_rec_wav(int device,int rate, int ch,
                                    const char *fn, unsigned *running);
