...
static char gkv_value[60];
char gkv_rx_test_config[80];
char gkv_tx_test_config[80];
char bename_rx_test_config[50];
...
int parse(struct test_params *commands)
{
    ...
    int vol_found = 0;
    char *temp;
    int params, sublen, val = 0;
    int volume_range = 0;
    const char s[2] = "-";
    char *split_gkv[30];
    char *split_gkv_rx[30];
    char *split_gkv_tx[30];
    ...
    while((p = fgets(array, sizeof(array), fp))) {
        ...
        if (!strncmp(&p[0], "#", 1)) {
            ...
            if ((temp = strstr(p,":"))) {
                ...
                temp++;
                temp[len] = '\0';
                strlcpy(gkv_rx_test_config, temp, strlen(temp)+1);
            }
        } else if (strstr(p, "gkv_tx")) {
            temp = NULL;
            if ((temp = strstr(p,":"))) {
                sublen = temp - p;
                len = len - sublen - 1;
                temp++;
                temp[len] = '\0';
                strlcpy(gkv_tx_test_config, temp, strlen(temp)+1);
            }
        } else if (strstr(p, "be_name_rx")) {
            temp = NULL;
            if ((temp = strstr(p,":"))) {
                sublen = temp - p;
                len = len - sublen - 1;
                temp++;
                temp[len] = '\0';
                strlcpy(bename_rx_test_config, temp, strlen(temp)+1);
            }
        } else if (strstr(p, "be_name_tx")) {
            temp = NULL;
            if ((temp = strstr(p,":"))) {
                sublen = temp - p;
                len = len - sublen - 1;
                temp++;
                temp[len] = '\0';
                strlcpy(bename_tx_test_config, temp, strlen(temp)+1);
            }
        } else if (strstr(p, "pcm_id_rx")) {
            temp = NULL;
            if ((temp = strstr(p,":"))) {
                sublen = temp - p;
                len = len - sublen - 1;
    ...
    while((p = fgets(array, sizeof(array), fp))) {
        ...
        break;
            ...
            if (len) {
                ...
                if (temp) {
                    sublen = temp - p;
                    DALSYS_memcpy(ctlname, p, sublen);
                    ctlname[sublen] = '\0';
                    ...
                    sublen--;
                    temp++;
                    DALSYS_memcpy(ctlval, temp, sublen);
                    ctlval[sublen] = '\0';
                    int val = -1;
                    while(sublen > 0) {
                        if (*temp == ' ') {
...
static DALBOOL ftm_audio_start_tone(struct test_params *params, uint16  lo_freq, uint16  hi_freq);
...
static DALBOOL ftm_audio_start_tone(struct test_params *params, uint16  lo_freq, uint16  hi_freq)
{
    ...
    if (params) {
        ftm_param.tone_play_param.dtmf_hi= params->fh;
        ftm_param.tone_play_param.dtmf_low=params->fl;
        ftm_param.tone_play_param.duration = params->duration;
    } else {
    ...
    ftm_param.tone_play_param.channels_rx=channels_rx_test_config;
    ftm_param.tone_play_param.device_id_rx=device_id_rx_test_config;

     if (strlen(gkv_rx_test_config) != 0) {
         ftm_param.tone_play_param.gkv_rx = malloc(strlen(gkv_rx_test_config)+1);
         if (ftm_param.tone_play_param.gkv_rx == NULL) {
             DALSYS_Log_Err("%s: Memory allocation failed\n",__func__);
             return -ENOMEM;
         }
         strlcpy(ftm_param.tone_play_param.gkv_rx, gkv_rx_test_config, strlen(gkv_rx_test_config)+1);
     } else {
         return FALSE;
     }

     if (strlen(bename_rx_test_config) != 0) {
         ftm_param.tone_play_param.be_name_rx = malloc(strlen(bename_rx_test_config)+1);
         if (ftm_param.tone_play_param.be_name_rx == NULL) {
             DALSYS_Log_Err("%s: Memory allocation failed\n",__func__);
             return -ENOMEM;
         }
         strlcpy(ftm_param.tone_play_param.be_name_rx, bename_rx_test_config, strlen(bename_rx_test_config)+1);
     } else {
         return FALSE;
     }
...
static DALBOOL ftm_audio_start_dsp_loopback()
{
    ...
    ftm_param.afe_lp_param.channels_rx = channels_rx_test_config;
    ftm_param.afe_lp_param.channels_tx = channels_tx_test_config;
    ftm_param.afe_lp_param.device_id_rx = device_id_rx_test_config;
    ftm_param.afe_lp_param.device_id_tx = device_id_tx_test_config;

    if(strlen(gkv_rx_test_config) != 0)
    {
        ftm_param.afe_lp_param.gkv_rx = malloc(strlen(gkv_rx_test_config)+1);
        if (ftm_param.afe_lp_param.gkv_rx == NULL) {
            DALSYS_Log_Err("%s: Memory allocation failed\n",__func__);
            return -ENOMEM;
        }
        strlcpy(ftm_param.afe_lp_param.gkv_rx, gkv_rx_test_config, strlen(gkv_rx_test_config)+1);
    }
    else
        goto error_close;

    if (strlen(gkv_tx_test_config) != 0)
    {
        ftm_param.afe_lp_param.gkv_tx = malloc(strlen(gkv_tx_test_config)+1);
        if (ftm_param.afe_lp_param.gkv_tx == NULL) {
            DALSYS_Log_Err("%s: Memory allocation failed\n",__func__);
            return -ENOMEM;
        }
        strlcpy(ftm_param.afe_lp_param.gkv_tx, gkv_tx_test_config, strlen(gkv_tx_test_config)+1);
    }
    else
        goto error_close;
    ...
    if (strlen(bename_rx_test_config) != 0)
    {
        ftm_param.afe_lp_param.be_name_rx = malloc(strlen(bename_rx_test_config)+1);
        if (ftm_param.afe_lp_param.be_name_rx == NULL) {
            DALSYS_Log_Err("%s: Memory allocation failed\n",__func__);
            return -ENOMEM;
        }
    strlcpy(ftm_param.afe_lp_param.be_name_rx, bename_rx_test_config, strlen(bename_rx_test_config)+1);
    }
    else
        goto error_close;

    if (strlen(bename_tx_test_config) != 0)
    {
        ftm_param.afe_lp_param.be_name_tx = malloc(strlen(bename_tx_test_config)+1);
        if (ftm_param.afe_lp_param.be_name_tx == NULL) {
            DALSYS_Log_Err("%s: Memory allocation failed\n",__func__);
            return -ENOMEM;
        }
    strlcpy(ftm_param.afe_lp_param.be_name_tx, bename_tx_test_config, strlen(bename_tx_test_config)+1);
    }
    else
        goto error_close;
...

{
    ...
    ftm_param.ext_lp_param.channels_rx = channels_rx_test_config;
    ftm_param.ext_lp_param.channels_tx = channels_tx_test_config;
    ftm_param.ext_lp_param.device_id_rx = device_id_rx_test_config;
    ftm_param.ext_lp_param.device_id_tx = device_id_tx_test_config;

    if (strlen(gkv_rx_test_config) != 0)
    {
        ftm_param.ext_lp_param.gkv_rx = malloc(strlen(gkv_rx_test_config)+1);
        if (ftm_param.ext_lp_param.gkv_rx == NULL) {
            DALSYS_Log_Err("%s: Memory allocation failed\n",__func__);
            return -ENOMEM;
        }
    strlcpy(ftm_param.ext_lp_param.gkv_rx, gkv_rx_test_config, strlen(gkv_rx_test_config)+1);
    }
    else
        goto error_close;
    ...
    if (strlen(gkv_tx_test_config) != 0)
    {
        ftm_param.ext_lp_param.gkv_tx = malloc(strlen(gkv_tx_test_config)+1);
        if (ftm_param.ext_lp_param.gkv_tx == NULL) {
            DALSYS_Log_Err("%s: Memory allocation failed\n",__func__);
            return -ENOMEM;
        }
    strlcpy(ftm_param.ext_lp_param.gkv_tx, gkv_tx_test_config, strlen(gkv_tx_test_config)+1);
    }
    else
        goto error_close;
    ...
    if (strlen(bename_rx_test_config)!= 0)
    {
       ftm_param.ext_lp_param.be_name_rx = malloc(strlen(bename_rx_test_config)+1);
       if (ftm_param.ext_lp_param.be_name_rx == NULL) {
           DALSYS_Log_Err("%s: Memory allocation failed\n",__func__);
           return -ENOMEM;
       }
    strlcpy(ftm_param.ext_lp_param.be_name_rx, bename_rx_test_config, strlen(bename_rx_test_config)+1);
    }
    else
        goto error_close;
    ...
    if (strlen(bename_tx_test_config) != 0)
    {
        ftm_param.ext_lp_param.be_name_tx = malloc(strlen(bename_tx_test_config)+1);
        if (ftm_param.ext_lp_param.be_name_tx == NULL) {
            DALSYS_Log_Err("%s: Memory allocation failed\n",__func__);
            return -ENOMEM;
        }
    strlcpy(ftm_param.ext_lp_param.be_name_tx, bename_tx_test_config, strlen(bename_tx_test_config)+1);
    }
    else
        goto error_close;
...
static DALBOOL ftm_audio_start_pcm_capture(uint8 seconds, const char *filename)
{
  ...
  ftm_param.pcm_rec_param.channels_tx=channels_tx_test_config;
  ftm_param.pcm_rec_param.device_id_tx=device_id_tx_test_config;

  if (strlen (gkv_tx_test_config) != 0)
  {
      ftm_param.pcm_rec_param.gkv_tx = malloc(strlen(gkv_tx_test_config)+1);
      if (ftm_param.pcm_rec_param.gkv_tx == NULL) {
          DALSYS_Log_Err("%s: Memory allocation failed\n",__func__);
          return -ENOMEM;
      }
  strlcpy(ftm_param.pcm_rec_param.gkv_tx, gkv_tx_test_config, strlen(gkv_tx_test_config)+1);
  }
  else
      goto error_close;
  ...
  if (strlen(bename_tx_test_config) != 0)
  {
      ftm_param.pcm_rec_param.be_name_tx = malloc(strlen(bename_tx_test_config)+1);
      if (ftm_param.pcm_rec_param.be_name_tx == NULL) {
          DALSYS_Log_Err("%s: Memory allocation failed\n",__func__);
          return -ENOMEM;
      }
  strlcpy(ftm_param.pcm_rec_param.be_name_tx, bename_tx_test_config, strlen(bename_tx_test_config)+1);
  }
  else
      goto error_close;
...
static DALBOOL ftm_audio_start_file_play(struct test_params *params)
{
    ...
    ftm_param.pcm_play_param.channels_rx=channels_rx_test_config;
    ftm_param.pcm_play_param.device_id_rx=device_id_rx_test_config;

    if (strlen(gkv_rx_test_config) != 0)
    {
        ftm_param.pcm_play_param.gkv_rx = malloc(strlen(gkv_rx_test_config)+1);
        if (ftm_param.pcm_play_param.gkv_rx == NULL) {
            DALSYS_Log_Err("%s: Memory allocation failed\n",__func__);
            return -ENOMEM;
        }
    strlcpy(ftm_param.pcm_play_param.gkv_rx, gkv_rx_test_config, strlen(gkv_rx_test_config)+1);
    }
    else
        goto error_close;
    ...
    if (strlen(bename_rx_test_config) != 0)
    {
        ftm_param.pcm_play_param.be_name_rx = malloc(strlen(bename_rx_test_config)+1);
        if (ftm_param.pcm_play_param.be_name_rx == NULL) {
            DALSYS_Log_Err("%s: Memory allocation failed\n",__func__);
            return -ENOMEM;
        }
    strlcpy(ftm_param.pcm_play_param.be_name_rx, bename_rx_test_config, strlen(bename_rx_test_config)+1);
    }
    else
        goto error_close;
...
static int test_ftm_tone_play_comm(uint32 path, struct test_params *params)
{
    ...
    if (!params) {
        if (!ftm_audio_start_tone(params, 300, 2000)) {
               result = 2;
               goto done;
        }
    } else {
        ...
        } else if (!ftm_audio_start_tone(params, 0, 0)) {
            result = 2;
            goto done;
        }
    }
    ...
    case FTM_AUDIO_TONES_PLAY:
        ...
        if(!ftm_audio_checkif_active_session() && g_current_path.bALLOW_TONE_PLAY)
        {
            ...
            if(!ftm_audio_start_tone(
                &cmd_ptr->audio_params.tone_params, 0, 0))
            {
    ...
    int tone_analysis, int freq_tolerance)
{
    ...
    if (codec == CODEC_TABLA)
        DALSYS_memcpy(ftm_tc_devices, ftm_tc_devices_tabla,
        sizeof(ftm_tc_devices_tabla));
    else if (codec == CODEC_SITAR)
        DALSYS_memcpy(ftm_tc_devices, ftm_tc_devices_sitar,
        sizeof(ftm_tc_devices_sitar));
    else if (codec == CODEC_TAIKO)
        DALSYS_memcpy(ftm_tc_devices, ftm_tc_devices_taiko,
        sizeof(ftm_tc_devices_taiko));

    else if (codec == CODEC_TAIKO_I2S) {
        DALSYS_memcpy(ftm_tc_devices, ftm_tc_devices_taiko_i2s,
