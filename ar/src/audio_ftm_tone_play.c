...
#define AUD_FTM_TONE_PLAY_PERIOD_SIZE   1024
/*==================================================================================================
...
static void aud_ftm_play_tone( void * pCtxt )
{
  ...
  do
      ...
      if( pDrvCtxt->bStart != TRUE ) break;
          ...
          if(size <= (capacity/2))
          {
            DALSYS_EventCtrl(pDrvCtxt->hReq_Data_Event);
          }
...
)
{
  ...
  /************* Create Critical Section ***********/
   ...
   if(DAL_SUCCESS != DALSYS_EventCreate(&(pDriverCtxt->hReq_Data_Event)))
   {
   ...
   pDriverCtxt->pDevCtxt->channels = pDriverCtxt->client_param.channels_rx;
   pDriverCtxt->pDevCtxt->device_id = pDriverCtxt->client_param.device_id_rx;

   pDriverCtxt->pDevCtxt->gkv = malloc(strlen(pDriverCtxt->client_param.gkv_rx)+1);
   if(pDriverCtxt->pDevCtxt->gkv == NULL) {
        DALSYS_Log_Err("%s: Memory allocation failed\n",__func__);
        return -ENOMEM;
   }

   strlcpy(pDriverCtxt->pDevCtxt->gkv, pDriverCtxt->client_param.gkv_rx, strlen(pDriverCtxt->client_param.gkv_rx) + 1);
   pDriverCtxt->pDevCtxt->be_name = malloc(strlen(pDriverCtxt->client_param.be_name_rx)+1);
   if(pDriverCtxt->pDevCtxt->be_name == NULL) {
        DALSYS_Log_Err("%s: Memory allocation failed\n",__func__);
        return -ENOMEM;
   }

   strlcpy(pDriverCtxt->pDevCtxt->be_name, pDriverCtxt->client_param.be_name_rx, strlen(pDriverCtxt->client_param.be_name_rx)+1);
   ...
   pDeviceCtxt->period_size = AUD_FTM_TONE_PLAY_PERIOD_SIZE;
...
)
{
  ...
  if(res != AUDIO_FTM_SUCCESS)
  {
    ...
    audio_ftm_hw_iocontrol(pDrvCxt->pDevCtxt, IOCTL_AUDIO_FTM_RX_DEV_BUF_SIZE, NULL, (uint8 *)&dev_buf_size);
    pDrvCxt->nRx_blk_convey_samples=dev_buf_size;
  ...
  /* buffer for fetch data from FIFO and pass to low layer driver for consuming */
  if(DAL_SUCCESS != DALSYS_Malloc(dev_buf_size*sizeof(int16), (void **)&pAud_ftm_rx_buf))
  {
          DALSYS_SyncLeave(pDrvCxt->hClientSync);
    ...
    uint8 * pBufIn                            /* Input: buffer pointer for input parameters */)
{
    ...
    res=pthread_create(&(pDrvCtxt->producer_thread), NULL,
        (void *(* _Nonnull)(void *))aud_ftm_tone_generator,(void *)pDrvCtxt);
        if(res !=0 )
        {
    ...
    res=pthread_create(&(pDrvCtxt->consumer_thread), NULL,
        (void *(* _Nonnull)(void *))aud_ftm_play_tone,(void *)pDrvCtxt);
        if(res !=0 )
        {
