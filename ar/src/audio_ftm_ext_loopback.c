...
#include "audio_ftm_ext_loopback.h"
...
typedef struct Aud_FTM_DrvCtxt_S{
    ...
    AUD_FTM_EXT_LP_PARAM_T      client_param;       /* parameters passed from client */
...
)
{
    ...
    ext_lp_param = (AUD_FTM_EXT_LP_PARAM_T *)pParam;
    printf("\n%s,freq=%d\n", __func__, ext_lp_param->freq);
    pDeviceCtxt->capturedevice = ext_lp_param->capturedevice;
    pDeviceCtxt->playbackdevice = ext_lp_param->playbackdevice;
    ...
    DALSYS_memcpy((uint8*)&(pDriverCtxt->client_param),
                  (uint8*)pParam, sizeof(AUD_FTM_EXT_LP_PARAM_T));

   pDriverCtxt->pDevCtxt->channels_rx = pDriverCtxt->client_param.channels_rx;
   pDriverCtxt->pDevCtxt->device_id_rx = pDriverCtxt->client_param.device_id_rx;

   pDriverCtxt->pDevCtxt->gkv_rx = malloc(strlen(pDriverCtxt->client_param.gkv_rx)+1);
   if(pDriverCtxt->pDevCtxt->gkv_rx == NULL) {
      DALSYS_Log_Err("%s: Memory allocation failed\n",__func__);
      return -ENOMEM;
   }

   strlcpy(pDriverCtxt->pDevCtxt->gkv_rx, pDriverCtxt->client_param.gkv_rx, strlen(pDriverCtxt->client_param.gkv_rx)+1);

   pDriverCtxt->pDevCtxt->be_name_rx = malloc(strlen(pDriverCtxt->client_param.be_name_rx)+1);
   if(pDriverCtxt->pDevCtxt->be_name_rx == NULL) {
      DALSYS_Log_Err("%s: Memory allocation failed\n",__func__);
      return -ENOMEM;
   }

   strlcpy(pDriverCtxt->pDevCtxt->be_name_rx, pDriverCtxt->client_param.be_name_rx, strlen(pDriverCtxt->client_param.be_name_rx)+1);

   pDriverCtxt->pDevCtxt->channels_tx = pDriverCtxt->client_param.channels_tx;
   pDriverCtxt->pDevCtxt->device_id_tx = pDriverCtxt->client_param.device_id_tx;

   pDriverCtxt->pDevCtxt->gkv_tx = malloc(strlen(pDriverCtxt->client_param.gkv_tx)+1);
   if(pDriverCtxt->pDevCtxt->gkv_tx == NULL) {
      DALSYS_Log_Err("%s: Memory allocation failed\n",__func__);
      return -ENOMEM;
   }

   strlcpy(pDriverCtxt->pDevCtxt->gkv_tx, pDriverCtxt->client_param.gkv_tx, strlen(pDriverCtxt->client_param.gkv_tx)+1);

   pDriverCtxt->pDevCtxt->be_name_tx = malloc(strlen(pDriverCtxt->client_param.be_name_tx)+1);
   if(pDriverCtxt->pDevCtxt->be_name_tx == NULL) {
      DALSYS_Log_Err("%s: Memory allocation failed\n",__func__);
      return -ENOMEM;
   }

   strlcpy(pDriverCtxt->pDevCtxt->be_name_tx, pDriverCtxt->client_param.be_name_tx, strlen(pDriverCtxt->client_param.be_name_tx)+1);
    ...
    hw_param.m_loopback_type = AUDIO_FTM_EXT_LOOPBACK;
...
)
{
    ...
    DALSYS_Free(pExplicitOpenContext);
    pExplicitOpenContext=NULL;
...
==================================================================================================*/
AUDIO_FTM_STS_T
audio_ftm_ext_lp_read(AUDIO_FTM_CLIENT_HANDLE_T clientHandle, void *pBuffer, uint32 nBufSize, uint32 *pCount)
{
    return AUDIO_FTM_SUCCESS;
}
...
==================================================================================================*/
AUDIO_FTM_STS_T
audio_ftm_ext_lp_write(AUDIO_FTM_CLIENT_HANDLE_T, void *, uint32 nBufSize)
{
    return AUDIO_FTM_SUCCESS;
}
...
)
{
    ...
    case IOCTL_AUDIO_FTM_CHG_VOL:
    {
        ...
        vol = *(uint16 *)pBufIn;
        audio_ftm_hw_iocontrol(pDevCtxt, IOCTL_AUDIO_FTM_CHG_VOL,(uint8 *) &vol, NULL);
    }
    break;
    ...
    case IOCTL_AUDIO_FTM_START:
    {
        ...
        dir=AUDIO_IF_SOURCE;
        audio_ftm_hw_iocontrol(pDevCtxt, IOCTL_AUDIO_FTM_START, (uint8 *)&dir, NULL);
        audio_ftm_ext_loopback_en(1, EXT_LOOP_TYPE, pDevCtxt, &pDevCtxt->pDrvCtxt->client_param);
    }
    break;

    case IOCTL_AUDIO_FTM_STOP:
    {
        /* pcm_close is called at each thread respectively. */
        audio_ftm_ext_loopback_en(0, EXT_LOOP_TYPE, pDevCtxt, &pDevCtxt->pDrvCtxt->client_param);
    }
    break;
