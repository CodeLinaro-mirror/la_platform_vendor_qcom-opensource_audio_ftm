...
static pthread_cond_t ftm_pcm_play_cond;
static pthread_mutex_t ftm_pcm_play_mutex;
...
static void aud_ftm_play_pcm( void * pCtxt )
{
    ...
    fd = open(pDrvCtxt->client_param.pFileName,O_RDONLY, 0444);
    if (fd < 0) {
       DALSYS_Log_Err("cannot open PCM play file\n");
       goto ftm_play_err;
    }

    if (lseek(fd, sizeof(struct wav_header), SEEK_SET) != sizeof(struct wav_header)) {
        DALSYS_Log_Err("%s: Seek failed\n", __func__);
        goto ftm_play_err;
    }
    ...
    pthread_mutex_lock(&ftm_pcm_play_mutex);
    pthread_cond_signal(&ftm_pcm_play_cond);
    pthread_mutex_unlock(&ftm_pcm_play_mutex);
...
audio_ftm_pcm_parse_header(char *filename, struct wav_header *phdr)
{
    ...
    DALSYS_memcpy(phdr,&hdr,sizeof(hdr));
...
)
{
  ...
  /************* Context creation ***********/
    ...
    DALSYS_memset(&hw_param,0,sizeof(Aud_FTM_HW_INIT_PARAM_T));
  ...
  /************* Create Critical Section ***********/
   ...
   hw_param.duration= pDriverCtxt->client_param.duration;
...
)
{
    ...
    case IOCTL_AUDIO_FTM_START:
    {
      AFEDevAudIfDirType dir;
        int res;
        pthread_mutex_init(&ftm_pcm_play_mutex, NULL);
        pthread_cond_init(&ftm_pcm_play_cond, NULL);

        dir=AUDIO_IF_SINK;
        /* create Playback thread */
        res=pthread_create(&(pDrvCtxt->hPlay_thread), NULL,
            (void *(* _Nonnull)(void *))aud_ftm_play_pcm,
            (void *)pDrvCtxt);
        if(res !=0 ) {
    ...
    case IOCTL_AUDIO_FTM_STOP:
    {
      ...
      duration = pDrvCtxt->client_param.duration;
      AFEDevAudIfDirType dir;
      dir=AUDIO_IF_SINK;
      pthread_mutex_lock(&ftm_pcm_play_mutex);
      gettimeofday(&now, NULL);
      timeout_time.tv_sec = now.tv_sec + duration;
      timeout_time.tv_nsec = now.tv_usec * 1000;
      pthread_cond_timedwait(&ftm_pcm_play_cond, &ftm_pcm_play_mutex,
          &timeout_time);
      pthread_mutex_unlock(&ftm_pcm_play_mutex);
