...
static void aud_ftm_record_pcm( void * pCtxt )
{
  int32  size, total, rd_len;
    ...
    if (pDevCtxt->capturedevice >= 0) {
        device_num = pDevCtxt->capturedevice;
    }
    audio_ftm_hw_rec_wav(device_num, 48000, 1, pDrvCtxt->client_param.pFileName, &pDrvCtxt->bStart);
#else
...
)
{
  ...
  /************* Create Critical Section ***********/
   ...
   pDriverCtxt->pDevCtxt->channels = pDriverCtxt->client_param.channels_tx;
   pDriverCtxt->pDevCtxt->device_id = pDriverCtxt->client_param.device_id_tx;

  pDriverCtxt->pDevCtxt->gkv = malloc(strlen(pDriverCtxt->client_param.gkv_tx)+1);
  if(pDriverCtxt->pDevCtxt->gkv == NULL) {
     DALSYS_Log_Err("%s: Memory allocation failed\n",__func__);
     return -ENOMEM;
  }

  strlcpy(pDriverCtxt->pDevCtxt->gkv, pDriverCtxt->client_param.gkv_tx, strlen(pDriverCtxt->client_param.gkv_tx)+1);

  pDriverCtxt->pDevCtxt->be_name = malloc(strlen(pDriverCtxt->client_param.be_name_tx)+1);
  if(pDriverCtxt->pDevCtxt->be_name == NULL) {
     DALSYS_Log_Err("%s: Memory allocation failed\n",__func__);
     return -ENOMEM;
  }

   strlcpy(pDriverCtxt->pDevCtxt->be_name, pDriverCtxt->client_param.be_name_tx, strlen(pDriverCtxt->client_param.be_name_tx)+1);
...
)
{
  ...
  if(res != AUDIO_FTM_SUCCESS)
  {
    ...
    audio_ftm_hw_iocontrol(pDrvCxt->pDevCtxt, IOCTL_AUDIO_FTM_TX_DEV_BUF_SIZE, NULL, (uint8 *)&dev_buf_size);
...
)
{
    ...
    res=pthread_create(&(pDrvCtxt->hRecord_thread), NULL,
        (void *(* _Nonnull)(void *))aud_ftm_record_pcm,
        (void *)pDrvCtxt);
        if(res !=0 )
        {
