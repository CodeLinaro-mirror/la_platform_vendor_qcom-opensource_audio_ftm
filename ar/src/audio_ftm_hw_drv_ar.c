...
int configure_mux(struct mixer *mixer, unsigned int device, unsigned int channels, char *intf_name)
{

    char *stream = "PCM";
    char *control = "setParamTag";
    char *s_control = "control";
    char *mixer_str = NULL;
    struct mixer_ctl *ctl;
    ...
    printf("%s configure mux for be %s\n", __func__, intf_name);

    ctl_len = strlen(stream) + 4 + strlen(s_control) + 1;
    mixer_str = calloc(1, ctl_len);
    if (!mixer_str) {
        printf("mixer_str calloc failed\n");
        return -ENOMEM;
    }
    snprintf(mixer_str, ctl_len, "%s%d %s", stream, device, s_control);
    ...
    tag_config->kv[0].value = slot_mask;
...
int audio_ftm_hw_loopback_en(int enable, int afe, Aud_FTM_DevCtxt_T *pDevCtxt)
{
	struct mixer *m = 0;
	...
	if (enable && afe) {
		...
		ret = pthread_create(&lb_play_thread, NULL,
            (void *(* _Nonnull)(void *))play_loopback, (void *)pDevCtxt);
		if (ret != 0)
		{
...
int audio_ftm_ext_loopback_en(int enable, int afe, Aud_FTM_DevCtxt_T *pDevCtxt,
                              AUD_FTM_EXT_LP_PARAM_T *client_param)
{
    struct mixer *m = 0;
    struct mixer_ctl *ctl = 0;
    int ret = 0;
    pthread_t lb_play_thread;
    pthread_t lb_rec_thread;
    ext_lp_param.freq = client_param->freq;
    ext_lp_param.rec_filename = client_param->filename;
    ext_lp_param.tone_analysis = client_param->tone_analysis;
    ext_lp_param.freq_tolerance = client_param->freq_tolerance;
    ext_lp_param.pDevCtxt = pDevCtxt;
		...
		(pDevCtxt->m_state == AUDIO_FTM_HW_DRV_STOPPED))
	{
		...
		case IOCTL_AUDIO_FTM_START:
		{
				...
				&& (pDevCtxt->rx_dev_id != -ENODEV))
			{
				if(pDevCtxt->m_loopback_type == AUDIO_FTM_AFE_LOOPBACK)
				{
					audio_ftm_hw_loopback_en(1, 1, pDevCtxt);
				}
				else if(pDevCtxt->m_loopback_type == AUDIO_FTM_ADIE_LOOPBACK)
				{
					audio_ftm_hw_loopback_en(1, 0, pDevCtxt);
				}
		...
		case IOCTL_AUDIO_FTM_STOP:
		{
			...
			if((pDevCtxt->bLoopbackCase == TRUE)
				...
				if(pDevCtxt->m_loopback_type == AUDIO_FTM_AFE_LOOPBACK)
				{
					audio_ftm_hw_loopback_en(0, 1, pDevCtxt);
					sleep(1);
				}
				else if(pDevCtxt->m_loopback_type == AUDIO_FTM_ADIE_LOOPBACK)
				{
					audio_ftm_hw_loopback_en(0, 0, pDevCtxt);
				}
			} else if (g_curr_rx_device == AUDIO_FTM_TC_RX_FM_STEREO_OUTPUT) {
